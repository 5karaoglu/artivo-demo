const { getWeather, saveAppointmentRequest, saveMaivoRepresentativeRequest } = require("../utils");
...
const service = ({logger, makeService}) => {
  ...
  svc.on('session:new', (session, path) => {
    ...
    if (session.direction === 'inbound') {
      selectedAgentId = process.env.ELEVENLABS_AGENT_ID;
    }

    // Agent ID'nin ayarlandığından emin ol, yoksa aramayı sonlandır.
    if (!selectedAgentId) {
      sessionLogger.error({direction: session.direction}, 'No agent could be selected for the call. Hanging up.');
      return session.hangup().send();
    }
    ...
    session.locals.logger.info({session, path, from: session.from, to: session.to}, `New call session: ${session.call_sid}, from: ${session.from}, to: ${session.to}, direction: ${session.direction}`);
